...
# ================= 全局设置 Matplotlib 字体为 Times New Roman =================
...
def plot_global_topomaps(hc_orig, pt_orig, hc_darl, pt_darl, ch_names, model_name, dataset, cur_path):
    ...
    # 创建 2x2 的画布，稍微加宽一点 (11, 8) 以便在右侧容纳 colorbar
    ...
    # 增加垂直间距 (hspace) 和底部留白，为横向 Colorbar 腾出空间
    ...
    # 为第一行添加独立的 Colorbar (横向放置在第一行的正下方居中)
    # [left, bottom, width, height] -> X轴 0.3 到 0.7 居中，Y轴 0.50 位于两行图形之间
    ...
    for tick in cbar_orig.ax.get_xticklabels():  # 注意：横向变成了修改 X 轴刻度字体
    ...
    # 为第二行添加独立的 Colorbar (横向放置在第二行的正下方居中)
    # Y轴 0.08 位于图片的最底部
    ...
    for tick in cbar_darl.ax.get_xticklabels():  # 注意：横向变成了修改 X 轴刻度字体
        tick.set_fontfamily('Times New Roman')
